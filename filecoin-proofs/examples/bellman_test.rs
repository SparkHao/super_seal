...
use filecoin_proofs::{PrivateReplicaInfo, PublicReplicaInfo, generate_window_post, verify_window_post};
...
lazy_static! {
    ...
    pub static ref DO_WINDOW: bool = get_window_ornot_env();
}
...
fn get_window_ornot_env() -> bool {
    let def = false;
    let res = if let Ok(do_post) = env::var("DO_WINDOW") {
...
pub fn seal_commit<Tree: 'static + MerkleTreeTrait>() {
    ...
    if &(*SECTORS_FOR_COMMIT).read().unwrap().len() > &0 {
        ...
        info!("There are ((((({:?}))))) sectors is ready for prove/window post", &(*SECTORS_FOR_COMMIT).read().unwrap().len());
        let rng = &mut rand::thread_rng();

        let sectors = (*SECTORS_FOR_COMMIT).read().unwrap();
        let sector_id = sectors.get(0).unwrap();
        //let sector_id = &(*SECTORS_FOR_COMMIT).read().unwrap().get(0).unwrap();
        let cache_path_str = get_cache_path(*sector_id);
        let cache_path: PathBuf = Path::new(&cache_path_str).into();
        let r = rng.gen();
        //let r = [251, 221, 34, 116, 6, 100, 28, 160, 223, 0, 152, 56, 88, 140, 47, 51, 91, 239, 162, 197, 150, 53, 30, 146, 130, 65, 74, 96, 96, 23, 90, 88];
        SEAL_COMMIT_NUM.fetch_add(1, Ordering::SeqCst);
        let seq = SEAL_COMMIT_NUM.load(Ordering::SeqCst);
        info!("*******************************************");
        info!("Seal commit {:?} sectors start!", seq);
        info!("seed random value is: {:?}", r);
        info!("*******************************************");

        let seed: Ticket = r;

        let metas = (*METAS_FOR_COMMIT).read().unwrap();
        let meta = metas.get(&sector_id).unwrap();
        //let meta =  &(*METAS_FOR_COMMIT).read().unwrap().get(&sector_id).unwrap();
        ...
        if !(*DO_WINDOW) {
            ...
            info!("##########################");
            info!("##########################");
            info!("Seal commit {:?} sectors end!", seq);
            info!("##########################");
            info!("##########################");
            ...
            pub_replica_info.insert(SectorId::from(*sector_id), pub_replica);
            priv_replica_info.insert(SectorId::from(*sector_id), priv_replica);
            ...
            info!("verify_window_post result: {:?}", verify_result);
            info!("##########################");
            info!("##########################");
            info!("Window post {:?} sectors!", SEAL_COMMIT_NUM.load(Ordering::SeqCst));
            info!("##########################");
            info!("##########################");
        }
        //
        ...
        //     &proof,
        // ).unwrap();
        // info!("verify_winning_post challenge is {:?} result: {:?}", challenge, verify_result);
...
fn main() {
    ...
    let prove_num_worker: usize = get_numeric_env("COMMIT_COUNT".to_string(), 4);
    ...
    let do_post = get_window_ornot_env();
    ...
    let mut f = File::open(comm_d_path).unwrap();
    f.read(&mut comm_d).unwrap();

    let comm_r_path = format!("{}/comm_r", cache_path_str);
    let mut comm_r = [0u8; 32];

    let mut f = File::open(comm_r_path).unwrap();
    f.read(&mut comm_r).unwrap();

    let pre_commit = SealPreCommitOutput {
        comm_r,
        comm_d,
    };
    let meta = sector_meta {
    ...
    start_prove(prove_num_worker, prove_interval);
